...
const DebugBar = ({ userId, isAdmin, adminUid }) => {
  return (
    ...
    >
      ...
      <p>
        **管理员 ID (Environment __user_id):** {adminUid || 'N/A (未配置)'}
      </p>
      <p>
        **是否管理员:** {isAdmin ? '是' : '否'}
      </p>
...
const LinkModal = ({ isOpen, onClose, linkToEdit, section, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onSave({
      ...
      url,
      description,
      category: category.trim(), // 确保分类名没有多余空格
    });
    onClose();
  };
...
// 子组件：加载默认数据确认模态框
const LoadDefaultConfirmModal = ({ isOpen, onClose, onConfirm, isLoading }) => {
    if (!isOpen) return null;

    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 transition-opacity duration-300">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-3xl p-6 w-full max-w-sm">
                <h3 className="text-lg font-bold text-yellow-600 dark:text-yellow-400 mb-4 flex items-center">
                    <AlertTriangle className="w-5 h-5 mr-2" /> 警告: 覆盖数据
                </h3>
                <p className="text-gray-700 dark:text-gray-300 mb-6">
                    您确定要加载默认数据吗？**此操作将永久覆盖**所有现有导航数据。
                </p>
                <div className="flex justify-end gap-3">
                    ...
                    <button
                        onClick={onConfirm}
                        disabled={isLoading}
                        className="px-4 py-2 rounded-lg bg-yellow-600 text-white font-semibold hover:bg-yellow-700 transition flex items-center disabled:bg-yellow-400"
                    >
                        {isLoading ? <Loader className="w-4 h-4 mr-2 animate-spin" /> : <Download className="w-4 h-4 mr-2" />}
                        确认覆盖
                    </button>
                </div>
            </div>
...
const AdminPanel = ({ navData, onLoadDefaultData, onDeleteLink, onEditLink, onAddLink }) => {
  ...
  const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false); // 修复：删除确认模态框状态
  const [isLoadDefaultConfirmOpen, setIsLoadDefaultConfirmOpen] = useState(false); // 新增：加载默认数据确认状态
  const [linkToDelete, setLinkToDelete] = useState(null);
  const [isDefaultLoading, setIsDefaultLoading] = useState(false);
  
  // 处理新增/编辑模态框打开
  const handleOpenModal = (section, link = null) => {
    setLinkToEdit(link);
  ...
  // 修复：处理加载默认数据（替换 window.confirm）
  const handleLoadDefault = () => {
    setIsLoadDefaultConfirmOpen(true);
  }

  const handleConfirmLoadDefault = async () => {
    setIsLoadDefaultConfirmOpen(false);
  ...
  const handleLinkSave = async ({ id, name, url, description, category }) => {
    ...
    if (id) {
        // 编辑现有链接
        await onEditLink(id, name, url, description, trimmedCategory);
    } else {
        // 添加新链接
        await onAddLink(name, url, description, trimmedCategory);
    }
  };
  ...
  return (
    <div className="space-y-12">
      <div className="flex justify-end gap-4 border-b pb-4 border-gray-200 dark:border-gray-700">
        ...
        <button
          onClick={handleLoadDefault} // 触发确认模态框
        ...
        <div key={section.id} className="p-6 bg-gray-50 dark:bg-gray-700 rounded-xl shadow-lg border border-gray-100 dark:border-gray-600">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
             ...
             {/* 额外添加按分类新增按钮，方便用户快速添加 */}
            <button
                onClick={() => handleOpenModal(section)}
                className="text-sm px-3 py-1 rounded-lg bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-800 transition flex items-center"
            >
            ...
            {section.links.map((link) => (
              ...
              >
                ...
                <div className="flex space-x-2 ml-4">
                  <button
                    onClick={() => handleOpenModal(section, link)}
      ...
      {isDeleteConfirmOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 transition-opacity duration-300">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-3xl p-6 w-full max-w-sm">
            ...
            <p className="text-gray-700 dark:text-gray-300 mb-6">
                您确定要删除 **{linkToDelete?.linkId}** 链接吗？此操作无法撤销。
            </p>
            <div className="flex justify-end gap-3">
              <button
      ...
      {/* 加载默认数据确认模态框 */}
      <LoadDefaultConfirmModal
        isOpen={isLoadDefaultConfirmOpen}
        onClose={() => setIsLoadDefaultConfirmOpen(false)}
        onConfirm={handleConfirmLoadDefault}
...
const App = () => {
  ...
  const firebaseConfig = useMemo(() => {
    const errorMsg = '配置缺失。无法加载 Firebase 数据库配置。请联系应用管理员解决配置问题。';
  ...
  }, [adminId, configError, firebaseConfig]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setNavData(data.sort((a, b) => a.category.localeCompare(b.category))); // 在内存中排序，方便管理和显示
    }, (error) => {
  ...
  // -----------------------------------------------------------------------
  // 6. 导航数据管理函数 (Admin Only) - 确保逻辑正确且使用 useCallback
  // -----------------------------------------------------------------------

  /**
   * 查找链接及其所属的分类文档
   */
  const findLinkAndSection = useCallback((linkId) => {
  ...
  /**
   * 帮助函数：更新指定分类文档的 links 数组
   */
  const handleUpdateSectionLinks = useCallback(async (sectionId, newLinks) => {
  ...
  }, [db, isAdmin]); // 依赖 db 和 isAdmin

  /**
   * 核心管理功能：添加链接
   */
  const handleAddLink = useCallback(async (name, url, description, category) => {
    if (!db || !isAdmin) return;
    ...
    const trimmedCategory = category.trim();
    const targetSection = navData.find(s => s.category === trimmedCategory);
  ...
  }, [db, isAdmin, navData, handleUpdateSectionLinks]); // 依赖 navData

  /**
   * 核心管理功能：编辑链接
   */
  const handleEditLink = useCallback(async (linkId, name, url, description, newCategory) => {
    if (!db || !isAdmin) return;
    ...
    if (oldSection.category === trimmedNewCategory) {
        // 仅修改链接内容 (在原分类内)
        ...
        // 2. 将链接添加到新分类
        const newSection = navData.find(s => s.category === trimmedNewCategory);
        if (newSection) {
            // 新分类已存在：更新其 links 数组
  ...
  }, [db, isAdmin, navData, findLinkAndSection, handleUpdateSectionLinks]);

  /**
   * 核心管理功能：删除链接
   */
  const handleDeleteLink = useCallback(async (sectionId, linkId) => {
    if (!db || !isAdmin) return;
  ...
  }, [db, isAdmin, navData, handleUpdateSectionLinks]);
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'dark bg-gray-900' : 'bg-gray-50'}`}>
      <div className="pt-8 pb-16 transition-colors duration-300">
        ...
        {/* 调试信息栏 (仅在特定条件下显示) */}
        {db && <DebugBar userId={userId} isAdmin={isAdmin} adminUid={adminId} />}
            ...
            <div className="flex gap-4 items-center">
                ...
                {isAdmin ? (
                    <button 
                      ...
                      className="text-red-600 dark:text-red-400 font-bold border border-red-500 px-3 py-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-900 transition flex items-center"
                    >
                    ...
                    <button 
                      ...
                      className="text-blue-600 dark:text-blue-400 font-bold border border-blue-500 px-3 py-1 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900 transition flex items-center"
                    >
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-6 md:p-10 border border-gray-100 dark:border-gray-700">
            {navData.length === 0 && !dataError && !isAdmin ? (
                <div className="text-center py-16 text-gray-500 dark:text-gray-400">
                  <Info className="w-12 h-12 mx-auto mb-4" />
                  <p className="text-xl font-semibold">导航数据为空</p>
                  <p className="mt-2">请稍候，或联系管理员添加内容。</p>
                </div>
            ) : isAdmin ? (
              <AdminPanel
                navData={navData}
